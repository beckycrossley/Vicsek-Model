...
def burn_in(L,N,v0,R):
    """Calculate the longest burn in time(when sigma=0) before the system reaches steady.
    ...
    L: float
        Size of the container(L*L)
    N: int
        Number of birds in the container
    v0: float
        The constant velocity of the birds per time step
    R: float
        Radius within which for brids to search for neighbours
