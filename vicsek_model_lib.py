...
def cal_pdist(pos,L):
    ...
    """
    N = np.size(pos)[0]     # number of birds
    Dist = np.zeros((N,N))
    ...
    for i in range(1,N):
        for j in range(i):
            t1 = 0
            t2 = 0
            dx = pos[j,0] - pos[i,0]   # dx = x2 - x1
            dy = pos[j,1] - pos[i,1]   # dy = y2 - y1
            if dx > L/2:
                t1 = -L
            if dx < -L/2:
                t1 = L
...
def update_wop(state, L, v0, R, sigma): 
    ...
    for i in range(N):
        ...
        adj = np.where(A[i,:N] < R)[0] # indices of birds within radius R
        if len(adj) > 0:
            theta = state[adj,2] # angles of all adjacent preys
        ...
        else:  # if there is no bird within radius R, the ith bird keeps in the same direction as the last time step
            heading[i] = state[i,2]
        
    
    # Update angle of birds with new headings and noise term
    state[:,2] = heading + stats.norm(0,sigma).rvs(N)
    # Update positions(mod L) of birds
...
def update_pred(prey, predator, L, v0, v_predator, R, R_predator, R_run_away, sigma_prey, simga_predator): 
    ...
    I = np.argmin(A[N:,:N],axis=1)  # the index of the prey nearest to the predator
    D = np.amin(A[N:,:N],axis=1)  # the index of the prey nearest to the predator
    dead_prey_index = np.unique(I[D<(0.1)])
    ...
    for i in range(N):
        ...
        if len(adj) > 0:
            ...
            # Compute heading for ith prey
            heading[i] = np.arctan2(sum_sin, sum_cos)
        else:  # if there is no prey within radius R, the ith prey keeps in the same direction as the last time step
            heading[i] = prey[i,2]
    ...
    # Update positions(mod L) of preys
    ...
    predator[:,2] += stats.norm(0,simga_predator).rvs(M)
    # Update positions(mod L) of predators
